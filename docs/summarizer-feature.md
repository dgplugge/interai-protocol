...
- **Project selector dropdown** (`interai-hub` repo, 2026-04-27). The dispatch UI carries a `cboProject` editable ComboBox in `grpControls` below the RAG checkbox, with an active-project label rendered in the form's title bar (e.g., "Agent Hub — Project: InterAI-Protocol"). The dropdown is populated at session start by walking the configured journal directory (`mJournalPath`) for project subdirectories. The operator may pick an existing project or type a new project name; on focus loss (Leave event), an unknown name triggers a confirmation dialog asking whether to create a new project — this mitigates the ghost-project-from-typo trust risk identified during Round 1 of the design discussion. Confirming a project switch fires `ProjectChangedEvent` in the Presenter, which clears `Session.Messages` and refreshes the visual transcript so the new project starts with a clean conversation; the prompt textbox (`txtMessage`) is intentionally preserved so any in-progress message survives the switch. `FetchRagContext` and `DispatchContext.Project` both read from `AgentHubView.ActiveProject` instead of the prior hardcoded `"InterAI-Protocol"` fallback. Default `ActiveProject = "InterAI-Protocol"` preserves backward compatibility when no UI selection has been made. Known limitation (out of scope for the initial ship): `mJournalContext` is loaded across all projects at session start and is not refreshed on project switch — the first dispatch in a newly-selected project may still see pre-loaded cross-project context. Future enhancement.
